	...
	struct manifest_vm *vm, ffa_vm_id_t vm_id, struct mpool *ppool)
{
	...
	if (!sp_pkg_init(stage1_locked, &sp_pkg, &sp_pkg_start, &pm,
			 sp_pkg_addr, ppool)) {
		return ret;
	}

	assert(pm != NULL);
	assert(va_addr(sp_pkg_start) != 0U);

	if (vm_id != HF_PRIMARY_VM_ID &&
	    sp_pkg_get_mem_size(&sp_pkg) >= vm->secondary.mem_size) {
		dlog_error("Invalid package header or DT size.\n");
		goto out;
	}

	if (!fdt_init_from_ptr(&sp_fdt, pm, sp_pkg.pm_size)) {
	...
	boot_info_header =
		(struct ffa_boot_info_header *)ptr_from_va(sp_pkg_start);

	if (vm->partition.boot_info &&
	    !ffa_boot_info_node(&boot_info_node, sp_pkg_addr, boot_info_header,
				&sp_pkg)) {
		dlog_error("Failed to process boot information.\n");
	}

out:
	sp_pkg_deinit(stage1_locked, &sp_pkg, sp_pkg_start, ppool);
	return ret;
}
