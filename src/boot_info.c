			...
			struct partition_pkg *pkg)
{
	...
	struct ffa_boot_info_header *boot_info_header =
		(struct ffa_boot_info_header *)ptr_from_va(
	...
	if (fdt_read_property(boot_info_node, "ffa_manifest", &data) &&
		...
		const uint32_t pm_size =
			pa_difference(pkg->pm.end, pkg->pm.begin);
	...
	if (ret == true) {  // TODO do this once or per each descriptor if
			    // descriptor types not mutually exclusive
		/*
